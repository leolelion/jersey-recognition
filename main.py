...
def log_execution_time(step_name, start_time):
    ...
    elapsed_time = time.time() - start_time
    execution_times.append((step_name, elapsed_time))
    print(f"{step_name} took {elapsed_time:.2f} seconds")

def save_execution_times(file_path="execution_times.csv"):
...
def get_soccer_net_legibility_results(args, use_filtered = False, filter = 'sim', exclude_balls=True):
    ...
    log_execution_time("Legibility Classification", start_time)
...
def generate_json_for_pose_estimator(args, legible = None):
    ...
    log_execution_time("Generate JSON for Pose Estimator", start_time)
...
def train_parseq(args):
    if args.dataset == 'Hockey':
        ...
        command = f"conda run -n {config.str_env}  train.py +experiment=parseq dataset=real data.root_dir={data_root} trainer.max_epochs=25 " \
...
def hockey_pipeline(args):
    ...
    if args.pipeline['legible']:
        ...
        command = f"python3 legibility_classifier.py --data {root_dir} --arch resnet50 --trained_model {config.dataset['Hockey']['legibility_model']}"
        success = os.system(command) == 0
...
def soccer_net_pipeline(args):
    ...
    if args.pipeline['soccer_ball_filter']:
        ...
        log_execution_time("Soccer ball filter", start_time)

    # 1. generate and store features for each image in each tracklet
    if args.pipeline['feat']:
        start_time = time.time()
        print("Generate features")
        command = f"conda run --no-capture-output -n {config.reid_env} python {config.reid_script} --tracklets_folder {image_dir} --output_folder {features_dir}"
        success = os.system(command) == 0
        print("Done generating features")
        log_execution_time("Generate Features", start_time)
    ...
    if args.pipeline['filter'] and success:
        ...
        log_execution_time("Identify and remove outliers", start_time)
    ...
    if args.pipeline['legible'] and success:
        ...
        log_execution_time("Classify Legibilty", start_time)
    ...
    if args.pipeline['legible_eval'] and success:
        ...
        log_execution_time("Evaluating Legibility", start_time)
    ...
    if args.pipeline['pose'] and success:
        ...
        log_execution_time("Generating JSON for pose", start_time)
        ...
        if success:
            ...
            log_execution_time("detecting pose", start_time)
    ...
    if args.pipeline['crops'] and success:
        ...
        try:
            ...
            print(e)
            success = False
        log_execution_time("Generating Crops", start_time)
    ...
    if args.pipeline['combine'] and success:
        ...
        log_execution_time("combine results", start_time)
    ...
    if args.pipeline['eval'] and success:
        ...
        log_execution_time("Accuracy Evaluation", start_time)
...
if __name__ == '__main__':
    ...
    if not args.train_str:
        ...
        train_parseq(args)
    log_execution_time("SoccerNet Pipeline", start_time)
